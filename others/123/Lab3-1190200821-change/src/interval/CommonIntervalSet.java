...
public class CommonIntervalSet<L> implements IntervalSet<L> {
	...
	public void insert(long start, long end, L label) throws IntervalConflictException  {
		...
		for(Entry<L, Long> entry:startMap.entrySet()) {
			...
			Long entry_end = endMap.get(key);
			if(key.equals(label)) {
				if(start!=entry_start || end!=entry_end) {
					throw new IntervalConflictException(" label冲突：但是[start,end)不同");
				} else {
					flag=false;
				}
			} else {
				if(isValueAtRange(start, entry_start, entry_end) 
					...
					throw new IntervalConflictException("包括两个重叠的区间");
				}
			}
		}
		if(flag) {
...
//        throw new RuntimeException("not implemented");
	...
	public String toString() {
		return String.format(endMap.toString());
//        throw new RuntimeException("not implemented");
	}
}